-- Fix cached_search_text becoming NULL when both definition-level and valsi-level
-- rafsi are NULL. In PostgreSQL, text || NULL yields NULL, so definition text was
-- dropped from search for most lujvo/fu'ivla entries created after V153.
--
-- Also fix the valsi trigger branch: read definition/notes/selmaho from the
-- definitions row directly (not only via keywordmapping), so valsi updates
-- (e.g. cached_decomposition) do not wipe definition text for keywordless entries.
...
-- Backfill all definitions so existing keywordless / NULL-rafsi rows become searchable.
UPDATE definitions d
SET cached_search_text = sub.computed
FROM (
    SELECT d2.definitionid,
        LOWER(
            COALESCE(v.word, d2.cached_valsiword, '') || ' ' ||
            COALESCE(d2.rafsi, v.rafsi, '') || ' ' ||
            COALESCE(d2.definition, '') || ' ' ||
            COALESCE(d2.notes, '') || ' ' ||
            COALESCE(d2.selmaho, '') || ' ' ||
            COALESCE(kw.kw_text, '')
        ) AS computed
    FROM definitions d2
    JOIN valsi v ON v.valsiid = d2.valsiid
    LEFT JOIN LATERAL (
        SELECT COALESCE(string_agg(LOWER(n.word || ' ' || COALESCE(n.meaning, '')), ' '), '') AS kw_text
        FROM keywordmapping k
        JOIN natlangwords n ON k.natlangwordid = n.wordid
        WHERE k.definitionid = d2.definitionid
    ) kw ON true
) sub
WHERE d.definitionid = sub.definitionid;
