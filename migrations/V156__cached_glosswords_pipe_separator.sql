-- Change cached_glosswords to use '|' as separator so each gloss phrase is
-- stored whole (e.g. "computer program") rather than space-split into tokens.
-- This fixes false-positive exact-gloss ranking in semantic search where a
-- token like "computer" matched inside the compound phrase "computer program".
-- cached_search_text (used for substring/ILIKE search) is unaffected.

-- 1. Backfill all existing rows with pipe-separated whole gloss phrases.
...
-- 2. Replace the trigger function with the corrected version (pipe separator).
...
BEGIN
    ...
    IF target_definitionid IS NOT NULL THEN
        -- kw_gloss: pipe-separated lowercased whole gloss phrases (place=0 only)
        -- kw_text:  space-joined lowercased word+meaning for full-text cached_search_text
