<template>
  ...
  <div v-if="!isLoading" class="space-y-4">
    ...
    <div
      class="hidden sm:grid sm:grid-cols-6 gap-4 py-3 px-4 bg-gray-50 border border-gray-200 rounded-xl font-medium text-gray-700"
    >

      <div class="text-sm font-medium"> {{ t('languageList.tableHeader.tag') }} </div>

      <div class="text-sm font-medium"> {{ t('languageList.tableHeader.englishName') }} </div>

      <div class="text-sm font-medium"> {{ t('languageList.tableHeader.lojbanName') }} </div>

      <div class="text-sm font-medium"> {{ t('languageList.tableHeader.realName') }} </div>

      <div class="text-sm font-medium"> {{ t('languageList.tableHeader.forLojban') }} </div>

      <div class="text-sm font-medium"> {{ t('languageList.tableHeader.url') }} </div>

    </div>
     <!-- Mobile Cards / Desktop Rows -->
    <div
      class="space-y-4 sm:space-y-0 sm:border sm:border-gray-200 sm:rounded-xl sm:bg-white sm:divide-y sm:divide-gray-200 overflow-hidden"
    >
      ...
      >
        ...
        <div class="sm:hidden space-y-3">
          ...
          <div class="btn-group-forced flex flex-wrap" role="group">
             <RouterLink :to="`/?mode=dictionary&langs=${lang.id}`" class="ui-btn--read ui-btn--group-item"
          ...
          <div class="text-sm text-secondary-600">
            ...
            <div>
               <span class="font-medium">{{ t('languageList.realNameLabel') }}</span
              > {{ lang.real_name }}
            </div>

            <div>
               <span class="font-medium">{{ t('languageList.codeLabel') }}</span
              > {{ lang.tag }}
            </div>

            <div>
               <span class="font-medium">{{ t('languageList.lojbanLabel') }}</span
              > {{ lang.lojban_name }}
            </div>
        ...
        <div class="hidden sm:block text-sm text-primary-800">
           <RouterLink :to="`/?mode=dictionary&langs=${lang.id}`" class="ui-btn--read"
            > {{ lang.english_name }} </RouterLink
          >
        </div>

        <div class="hidden sm:block text-sm text-primary-800"> {{ lang.lojban_name }} </div>

        <div class="hidden sm:block text-sm text-primary-800"> {{ lang.real_name }} </div>

        <div class="hidden sm:block text-sm text-primary-800">
           <span
            v-if="lang.for_lojban"
            class="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-success-100 text-success-800"
            > {{ t('languageList.yes') }} </span
          >
        </div>
  ...
  <div
    v-if="!isLoading && languages.length === 0"
  ...
  <div
    v-if="!isLoading && languages.length > 0 && filteredLanguages.length === 0"
    class="text-center py-12 bg-gray-50 rounded-xl border border-gray-200"
  >
