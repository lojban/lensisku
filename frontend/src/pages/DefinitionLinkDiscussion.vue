            ...
            >
              ...
              <div class="mt-4">
                 <RouterLink
                  ...
                  > {{ t('definitionLinkDiscussion.viewEntry') }} <ArrowUpRight
                    class="ml-1 h-3 w-3"
                  /> </RouterLink
                >
              </div>
...
const loadComments = async () => {
  ...
  try {
    ...
    params.append('definition_link_id', String(props.id))
    params.append('page', String(currentPage.value))
    params.append('per_page', String(perPage.value))

    if (route.query.comment_id) params.append('comment_id', queryStr(route.query.comment_id))
    if (route.query.scroll_to) params.append('scroll_to', queryStr(route.query.scroll_to))
    ...
    if (route.query.scroll_to) {
      nextTick(() => scrollToComment(parseInt(queryStr(route.query.scroll_to), 10)))
    }
  } catch (error) {
