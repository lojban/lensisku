<template>
  ...
  <div class="collection-widget">
    <!-- Add to Collection Button --> <button class="ui-btn--empty flex items-center gap-2 hover:text-yellow-600"
      @click="openModal" :title="t('collectionWidget.addToCollection')">
      <CopyPlus class="w-4 h-4" />
    </button>
    <ModalComponent :show="showModal" :title="t('collectionWidget.modalTitle')" @close="closeModal"> <!-- Header -->
      <template #header>
        <h3 class="text-xl font-bold"> {{ t('collectionWidget.modalTitle') }} </h3>
      </template> <!-- Loading State -->
      <LoadingSpinner v-if="isLoading" variant="inline" class="py-4" /> <!-- Collections List -->
      ...
      <div v-else>
        ...
        <div v-if="collections.length === 0" class="px-3 py-4 text-center">
          ...
          <IconButton button-classes="ui-btn--create mt-4 mx-auto" :label="t('collectionWidget.createFirst')"
        ...
        <form class="space-y-3" @submit.prevent="createAndAddToCollection">
          ...
          <div>
            <label class="block text-xs font-medium text-gray-700 mb-1">{{
              ...
              }}</label> <textarea v-model="newCollection.description" rows="2" class="textarea-field" />
          </div>

          <div class="flex items-center space-x-2">
            <input id="is_public" v-model="newCollection.is_public" type="checkbox" class="checkbox-toggle" /> <label
              for="is_public" class="text-xs text-gray-700"> {{ t('collectionWidget.makePublic') }} </label>
          </div>
          ...
          <div class="mt-2 flex flex-col gap-2">
            ...
            <div class="flex justify-end gap-2">
              <button type="button" class="ui-btn--cancel" :disabled="isCreating" @click="showCreateForm = false">
                {{ t('collectionWidget.cancel') }} </button> <button type="submit" :disabled="isCreating"
                class="ui-btn--create">
                {{
      ...
      <div v-if="showNotesInput" class="border-t mt-2 pt-2">
        <label class="block text-xs font-medium text-gray-700 mb-1">{{
          ...
          }}</label> <textarea v-model="notes" rows="2" :placeholder="t('collectionWidget.notesPlaceholder')"
        ...
        <div class="mt-2 flex flex-col gap-2">
          ...
          <div class="flex justify-end gap-2">
            <button class="ui-btn--cancel" :disabled="isAddingTo === selectedCollectionId" @click="cancelAddWithNotes">
              {{ t('collectionWidget.cancel') }} </button> <button class="ui-btn--insert"
              :disabled="isAddingTo === selectedCollectionId" @click="confirmAddWithNotes">
