<template>
  ...
  >
    ...
    > <RouterLink to="/lingo/courses" class="ml-3 flex items-center gap-2"
      > <GraduationCap class="h-8 w-8 text-white" /> <span class="text-xl font-bold text-white">{{
        t('lingo.appName')
      }}</span
      > </RouterLink
    > <!-- Mobile sidebar overlay --> <Teleport to="body"
      ...
      > <Transition name="lingo-drawer-panel"
        ...
        >
          ...
          <div class="flex items-center justify-between border-b border-slate-200 p-4">
             <span class="text-lg font-bold text-green-600">Menu</span> <button
              type="button"
