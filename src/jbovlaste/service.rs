...
pub async fn semantic_search(
    ...
    if params.word_type.is_none() {
        ...
        // Native wiki pages (typeid 16) have their own search channel.
    ...
    if let Some(exclude_id) = params.exclude_definition_id {
        ...
        conditions.push(format!("AND d.definitionid <> ${}", query_params.len() + 1));
...
pub async fn semantic_graph_anchor_embedding_english_valsi(
    ...
    semantic_graph_valsi_embedding(
        pool,
        search_term,
        Some(&[SEMANTIC_GRAPH_ANCHOR_ENGLISH_LANGID]),
    )
    .await
}
...
pub async fn search_definitions(
    ...
    if params.word_type.is_none() {
        ...
        // Native wiki pages (typeid 16) have their own search channel.
        conditions.push("AND d.cached_typeid != 16".to_string());
    }
    ...
    if params.word_type.is_none() {
        ...
        // Native wiki pages (typeid 16) have their own search channel.
        count_conditions.push("AND d.cached_typeid != 16".to_string());
    }
...
/// Fetch the native wiki definition for a valsi word (typeid = 16).
pub async fn get_wiki_by_word(
    ...
    let row = transaction.query_opt(
        ...
        WITH image_check AS (
            SELECT EXISTS (
                SELECT 1 FROM definition_images di
                JOIN definitions d ON di.definition_id = d.definitionid
                JOIN valsi v ON d.valsiid = v.valsiid
...
async fn upsert_wiki_in_transaction(
    ...
    if source_langid == 1 || source_langid == 58 {
        validate_mathjax(&sanitized_definition, &options)
            .await
    ...
    let definition_id = if let Some(row) = existing_definition {
        ...
        transaction
            .execute(
                "UPDATE definitions
                 SET definition = $1, langid = $2, notes = NULL, etymology = NULL,
                     selmaho = NULL, jargon = NULL, time = $3, owner_only = $4,
                     embedding = NULL
    ...
    transaction
        .execute(
            ...
            &[
                &valsi_id,
                &format!("Wiki page updated for {}", valsi_word),
                &url,
                &claims.sub,
                &definition_id,
            ],
    ...
    if let Err(e) = redis_cache.invalidate_definition_search_caches().await {
        log::error!(
            "Failed to invalidate definition search caches after wiki upsert: {}",
            e
        );
    }
    if let Err(e) = redis_cache.invalidate_recent_changes().await {
        log::error!(
            "Failed to invalidate recent changes cache after wiki upsert: {}",
            e
        );
    }
...
fn wiki_redirect_from_metadata(metadata: Option<&serde_json::Value>) -> (bool, Option<String>) {
...
pub async fn rename_wiki_page(
    ...
    transaction
        .execute(
            ...
            &[&metadata, &(Utc::now().timestamp() as i32), &definition_id],
        )
        .await?;
...
pub async fn update_definition(
    ...
    let (notes, etymology, selmaho, jargon) = if request.is_wiki == Some(true) {
        (None, None, None, None)
    } else {
        (
            sanitized_notes.clone(),
            sanitized_etymology.clone(),
            sanitized_selmaho.clone(),
            sanitized_jargon.clone(),
        )
    };
    transaction
        .execute(
            ...
            {})",
                ...
                for row in base_changes {
                    ...
                    let mut change = RecentChange {
                        ...
                        language_lojban_name: row.get("language_lojban_name"),
                        diff: None,
